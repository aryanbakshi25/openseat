  ...
  try {
    ...
    await Promise.all(
      allLocationIds.map(async (lid) => {
        const res = await fetch(
          `${LIBCAL_BASE_URL}/space/bookings?lid=${lid}&date=${today}&days=7&limit=500&include_tentative=1`,
    ...
    const tentative = deduped.filter(
      (b) => b.status === "Mediated Tentative",
    );

    const enriched: StaffBooking[] = await Promise.all(
      tentative.map(async (b) => {
        const rawAccount = b.account || b.email || "";
        const username = rawAccount.replace(/@.*$/, "");

        const lookup = username ? await lookupPatron(username) : null;
        ...
        return {
          ...
          classification: classifyPatron(lookup),
        };
      }),
    );
